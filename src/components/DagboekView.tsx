...
const PhotoImg: React.FC<{
  photo: PhotoItem;
  alt?: string;
  className?: string;
}> = ({ photo, alt, className }) => {
  const src = useFileSrc(photo.fileId, photo.dataUrl || photo.url);
...
const isoDate = (value: string) => {
  ...
  if (parts.length === 3 && parts[2] > 1000) {
    return `${parts[2]}-${String(parts[1]).padStart(
      2,
      "0",
...
export const DagboekView: React.FC<DagboekViewProps> = ({
  ...
  const initialIndex = useMemo(() => {
    ...
    const exact = sortedDays.findIndex(
      (day) => day.date === today,
    );

    if (exact >= 0) return exact;

    const next = sortedDays.findIndex(
      (day) => day.date > today,
    );
  ...
  const journal = journals.find(
    (item) => isoDate(item.datum) === selectedDate,
  );
  ...
  const [stemming, setStemming] =
    useState<JournalEntry["stemming"]>("Dankbaar");
  ...
  const [photoActivity, setPhotoActivity] =
    useState("Reisdag");
    ...
    return (
      <div className="rounded-3xl border border-slate-200 bg-white p-8 text-center dark:border-slate-800 dark:bg-slate-900">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Voeg eerst dagen toe aan de tijdlijn.
        </p>
      </div>
    );
  }
  ...
  const submitJournal = (
    ...
    onAddJournal({
      ...
      stemming,
      hoogtepunt:
        hoogtepunt.trim() || "Niet ingevuld",
      dieptepunt: dieptepunt.trim() || "Geen",
      geleerdeLessen:
        les.trim() || "Niet ingevuld",
      mooisteFoto: dayPhotos[0]?.url ?? "",
      favorieteHerinnering:
        herinnering.trim() ||
        hoogtepunt.trim() ||
        "Niet ingevuld",
    });
  ...
  const submitPhoto = (
    ...
    const externalUrl = photoUrl.trim();

    if (
      (!photoFile && !externalUrl) ||
      !photoCaption.trim()
    ) {
      return;
    }
    ...
    onAddPhoto({
      ...
      /*
       * Bij een lokale foto blijft url leeg.
       * De daadwerkelijke foto staat in IndexedDB
       * en wordt via fileId teruggevonden.
       */
      url: photoFile ? "" : externalUrl,

      /*
       * NIEUWE OPSLAG:
       * alleen de IndexedDB-key bewaren.
       */
      ...
      plaats: selectedDay.plaats,
      activiteit:
        photoActivity.trim() || "Reisdag",
      albumName: `${selectedDay.land} – ${selectedDay.plaats}`,
      gps: selectedDay.gps,
    });

    /*
     * De preview-URL is alleen voor de tijdelijke
     * weergave tijdens het toevoegen.
     */
  ...
  const closePhotoForm = () => {
    /*
     * Als de gebruiker een foto heeft geselecteerd
     * maar nog niet heeft opgeslagen, verwijderen we
     * de tijdelijke IndexedDB-file.
     */
    if (photoFile?.fileId) {
      void deleteFile(photoFile.fileId).catch(
        (cause) =>
          console.error(
            "Concept-foto kon niet worden opgeruimd:",
            cause,
          ),
      );
    }
    ...
    /*
     * Ook de tijdelijke browser-preview opruimen.
     */
    if (photoFile?.previewUrl) {
      URL.revokeObjectURL(photoFile.previewUrl);
    }
  ...
  const handlePhotoFileChange = async (
    ...
    /*
     * Oude conceptfoto opruimen wanneer de gebruiker
     * eerst een andere foto had gekozen.
     */
    if (photoFile?.fileId) {
      try {
        await deleteFile(photoFile.fileId);
      } catch (cause) {
        console.error(
          "Oude conceptfoto kon niet worden verwijderd:",
          cause,
        );
      }
    }
    ...
    try {
      /*
       * BELANGRIJK:
       * De daadwerkelijke foto wordt direct in IndexedDB
       * opgeslagen. Geen dataUrl meer in de reisdata.
       */
      const meta = await storeOptimizedImage(file);

      if (!meta?.id) {
        throw new Error(
          "De foto is opgeslagen, maar er kon geen opslag-ID worden aangemaakt.",
        );
      }

      /*
       * Preview alleen tijdelijk in het geheugen.
       * Deze URL wordt NIET opgeslagen in de reisdata.
       */
      const previewUrl =
      ...
      setPhotoError(
        cause instanceof Error
          ? cause.message
          : "De foto kon niet worden opgeslagen. Mogelijk is de lokale opslag vol.",
      );
    } finally {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 flex items-center gap-1.5 text-sm text-slate-200">
                <MapPin className="h-4 w-4 text-[#39B8C8]" />
                {selectedDay.land}
              </p>
            </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <div className="space-y-6">
            ...
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="flex items-center gap-2 text-lg font-black text-slate-900 dark:text-white">
                  <BookOpen className="h-5 w-5 text-[#39B8C8]" />
                  Mijn verhaal
                </h3>

                <p className="mt-0.5 text-xs text-slate-500">
                  Verhaal, stemming en herinneringen van
                  deze reisdag.
                </p>
              </div>
              ...
              <button
                onClick={() => {
                  ...
                  setLes(
                    journal?.geleerdeLessen || "",
                  );
                  setHerinnering(
                    journal?.favorieteHerinnering ||
                      "",
                  );
                  setStemming(
                    journal?.stemming ||
                ...
                className="flex items-center gap-2 rounded-xl bg-[#174A7E] px-4 py-2.5 text-xs font-bold text-white hover:bg-[#1d5c9c]"
              >
                {journal ? (
                  <Pencil className="h-4 w-4 text-[#39B8C8]" />
                ) : (
                  <Plus className="h-4 w-4 text-[#39B8C8]" />
                )}
              ...
              {journal && (
                <button
                  ...
                  onClick={() =>
                    window.confirm(
                      "Dit dagboekverhaal verwijderen?",
                    ) &&
                    onDeleteJournal(journal.id)
                  }
                  className="flex items-center gap-2 rounded-xl border border-rose-200 px-3 py-2.5 text-xs font-bold text-rose-600"
                >
            ...
            {journal ? (
              <div className="space-y-5">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full bg-cyan-50 px-3 py-1.5 text-xs font-bold text-[#174A7E] dark:bg-cyan-950/40 dark:text-cyan-300">
                    {moodEmoji[journal.stemming]}{" "}
                    {journal.stemming}
                  </span>

                  <span className="rounded-full bg-amber-50 px-3 py-1.5 text-xs font-bold text-amber-700 dark:bg-amber-950/40 dark:text-amber-300">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <MemoryCard
                    title="Geleerd"
                    value={journal.geleerdeLessen}
                  />

                  <MemoryCard
                    title="Minder leuk"
                    value={journal.dieptepunt}
                  />
                </div>
              </div>
            ) : (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-5 py-9 text-center dark:border-slate-700 dark:bg-slate-800/40">
                ...
                <p className="font-bold text-slate-800 dark:text-white">
                  Deze dag heeft nog geen verhaal.
                </p>

                <p className="mt-1 text-xs text-slate-500">
                  De locatie en reisdatum worden automatisch
                  ingevuld.
                </p>
              </div>
            )}
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="flex items-center gap-2 text-lg font-black text-slate-900 dark:text-white">
                  <Camera className="h-5 w-5 text-[#39B8C8]" />
                  Foto’s van deze dag
                </h3>

                <p className="mt-0.5 text-xs text-slate-500">
                  Automatisch gegroepeerd op reisdatum.
                </p>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
              >
                <ImagePlus className="h-4 w-4 text-[#39B8C8]" />
                Foto toevoegen
              </button>
            </div>
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-3xl bg-[#174A7E] p-5 text-white shadow-sm">
            ...
            <a
              href={`https://www.google.com/maps?q=${selectedDay.gps.lat},${selectedDay.gps.lng}`}
              target="_blank"
              rel="noreferrer"
              className="mt-4 inline-flex rounded-xl bg-white/10 px-4 py-2 text-xs font-bold hover:bg-white/20"
            >
              Open in Google Maps
            </a>
          </section>
      ...
      {showJournalForm && (
        <Modal
          title={`Dagboek – ${selectedDay.plaats}`}
          onClose={() =>
            setShowJournalForm(false)
          }
        >
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2">
              <select
                ...
                onChange={(e) =>
                  setStemming(
                    e.target
                      .value as JournalEntry["stemming"],
                  )
                }
                className="field"
              >
                <option>Blij</option>
                <option>Dankbaar</option>
                <option>Moe</option>
              ...
              <input
                ...
                className="field"
                placeholder="Hoogtepunt van de dag"
              />
            </div>
            ...
            <textarea
              ...
              placeholder="Vertel wat jullie vandaag hebben meegemaakt…"
              required
            />
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <input
                value={herinnering}
                onChange={(e) =>
                  setHerinnering(e.target.value)
                }
                className="field"
                placeholder="Favoriete herinnering"
              />

              <input
                value={les}
                onChange={(e) =>
                  setLes(e.target.value)
                }
                className="field"
                placeholder="Wat hebben jullie geleerd?"
              />
            </div>
            ...
            <input
              value={dieptepunt}
              onChange={(e) =>
                setDieptepunt(e.target.value)
              }
              className="field"
              placeholder="Minder leuk moment (optioneel)"
            />

            <div className="flex justify-end">
              <button
                type="submit"
                className="flex items-center gap-2 rounded-xl bg-[#174A7E] px-5 py-2.5 text-xs font-bold text-white"
              >
      ...
      {showPhotoForm && (
        <Modal
          title={`Foto toevoegen – ${selectedDay.plaats}`}
        ...
        >
          <form
            ...
            className="space-y-3"
          >
            <label className="block rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm font-bold dark:border-slate-700 dark:bg-slate-800">
              Foto vanaf telefoon
              ...
              <input
                ...
                onChange={(event) => {
                  void handlePhotoFileChange(
                    event.target.files?.[0],
                  );

                  /*
                   * Zorg dat dezelfde foto opnieuw
            ...
            <details>
              <summary className="cursor-pointer text-xs font-bold text-slate-500">
                Of gebruik een afbeeldingslink
              </summary>
              ...
              <input
                ...
                placeholder="Afbeeldingslink (URL)"
              />
            </details>
            ...
            <input
              value={photoActivity}
              onChange={(e) =>
                setPhotoActivity(e.target.value)
              }
              className="field"
            ...
            <p className="text-[11px] text-slate-500">
              De datum, locatie, het land en album
              worden automatisch gekoppeld aan deze
              reisdag.
            </p>
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={photoBusy}
                className="flex items-center gap-2 rounded-xl bg-[#174A7E] px-5 py-2.5 text-xs font-bold text-white disabled:opacity-50"
              >
      ...
      {lightboxPhoto && (
        ...
        >
          <div
            className="relative max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-3xl bg-slate-950"
            onClick={(e) =>
              e.stopPropagation()
            }
          >
            ...
            <div className="p-5 text-white">
              ...
              <p className="mt-1 text-xs text-slate-400">
                {lightboxPhoto.activiteit} ·{" "}
                {lightboxPhoto.plaats}
              </p>
            </div>
          </div>
        </div>
